...
var cw = (canvas.width = bc.width = window.innerWidth),
  cx = cw / 2;
var ch = (canvas.height = bc.height = window.innerHeight + 100),
  cy = ch;
...
function Init() {
  ...
  (cw = canvas.width = bc.width = window.innerWidth), (cx = cw / 2);
  (ch = canvas.height = bc.height = window.innerHeight + 100), (cy = ch);
  bCtx.strokeStyle = "#abcdef";
  bCtx.lineWidth = 1;
  Draw();
}
